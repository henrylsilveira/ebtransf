...
export default function PopoverCarregarProcessos({ setProcessos }: { setProcessos: React.Dispatch<React.SetStateAction<ModeloProcessoProps[]>> }) {
    ...
    async function handleUpdateProcess() {
        try {
            setLoading(true)
            if (formData.token === "" || formData.locator === "") return toast.error('Insira o token e o localizador!', {
                ...
                <Popover.Content className="bg-gray-900 z-10 p-2 rounded-md shadow-shape flex justify-center flex-col ">
                    <div className="py-2 gap-2">
                        <div className='flex w-64 flex-col gap-2'>
                            <h1 className='flex justify-center uppercase font-semibold border-b border-green-800'>Carregar processos</h1>
                            <div className="relative z-0 w-full p-1 group">
                                <input type="text" name="token" id="token" onChange={handleChange} className="block py-2.5 px-0 w-full text-sm [appearance:textfield] text-white bg-transparent border-0 border-b-2 border-gray-300 dark:text-white dark:border-gray-600 dark:focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer" placeholder=" " required />
                            ...
                            <div className="relative z-0 w-full p-1 group">
                                <input type="text" name="locator" id="locator" onChange={handleChange} className="block py-2.5 px-0 w-full text-sm [appearance:textfield] text-white bg-transparent border-0 border-b-2 border-gray-300 dark:text-white dark:border-gray-600 dark:focus:border-green-500 focus:outline-none focus:ring-0 focus:border-green-600 peer" placeholder=" " required />
