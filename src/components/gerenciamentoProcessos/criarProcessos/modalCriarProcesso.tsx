...
import DropdownModelosProcessos from "./dropdownModelosProcessos";
import * as Accordion from "@radix-ui/react-accordion";
import * as Popover from "@radix-ui/react-popover";
import { NotData } from "@/components/NotData";
import { BsTrash3 } from "react-icons/bs";
export default function ModalCriarProcesso({
  setRegistrosModelos,
  registrosModelos,
}: {
  setRegistrosModelos: React.Dispatch<
    React.SetStateAction<ModeloProcessoProps[]>
  >;
  registrosModelos: ModeloProcessoProps[];
}) {
  ...
  const [formData, setFormData] = useState<ModeloProcessoProps>(
  ...
  function baixarModelos() {
    setLoading(true);
    var fileName = `${
      new Date().toLocaleString() + "-" + "modelosProcessos"
    }.ebcalcP`;

    // Create a blob of the data
    var fileToSave = new Blob([JSON.stringify(registrosModelos)], {
      type: "application/ebcalcP",
    });
  ...
  function importarModelos(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    function logFile(e: any) {
      setRegistrosModelos(JSON.parse(e.target.result));
    }
    try {
      const registros = JSON.stringify([...registrosModelos]);
      setRegistrosModelos(JSON.parse(registros));
      async () =>
        await new Promise((resolve) => {
          setTimeout(() => {
                      ...
                      <div className="py-2 gap-2 flex flex-col flex-1">
                        ...
                        >
                          ...
                          <Accordion.AccordionItem value="item-2">
                            <DropdownModelosProcessos
                              modelos={registrosModelos}
                              setModelos={setRegistrosModelos}
                            />
                          </Accordion.AccordionItem>
            ...
            <div>
              <div className="flex flex-col gap-4">
                ...
                <div className="bg-gray-950 shadow-shape flex flex-1 rounded-md items-center px-4 py-4 gap-4">
                  ...
                  <button
                    onClick={() => addEtapas(nomeEtapa)}
                    className="hover:bg-green-800 text-xs w-24 h-8 items-center flex justify-center bg-transparent border border-green-700 uppercase text-white  rounded-md"
                  >
                    Adicionar
                  </button>
                </div>
                <div className="text-white">
                  <h1 className="text-2xl text-white flex justify-center items-center">
                    {formData["titulo"] && (
                      <MdOutlineKeyboardDoubleArrowRight className="text-green-600 w-5 h-5" />
                    )}
                    {formData["titulo"]}
                  </h1>
                  <ul>
                    {etapas.map((etapa, index) => (
                      <li
                        className="bg-gray-950 shadow-shape flex flex-1 rounded-md items-center py-1 px-2 justify-between"
                        key={index}
                      >
