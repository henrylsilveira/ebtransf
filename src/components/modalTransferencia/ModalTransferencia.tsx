  ...
  return (
    <AlertDialog.Root open={open} onOpenChange={setOpen}>
      <AlertDialog.Trigger asChild>
        <button className="flex shadow-container justify-center items-center px-4 text-white/80 bg-green-900/20 hover:bg-green-600/30 border border-green-400 rounded-lg text-2xl">
          <FaEye />
        </button>
      </AlertDialog.Trigger>
      <AlertDialog.Portal>
        ...
        <AlertDialog.Content className="data-[state=open]:animate-contentShow overflow-y-auto fixed top-[50%] left-[50%] max-h-[85vh] w-[80vw] max-w-[800px] translate-x-[-50%] translate-y-[-50%] rounded-[6px] bg-backgroundColor border border-green-700 p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none">
          <AlertDialog.Title className="sm-0 text-xl font-medium">
            ...
            <div className="flex flex-col justify-between sm:flex-row">
              <div className="flex flex-col gap-2 ">
                <p className="flex gap-2 text-sm sm:text-2xl text-white">
                  <TbPlaneDeparture className="text-green-800" />
                  {cidadeOrigem} / {estadoOrigem}
                </p>
                <p className="flex gap-2 text-sm sm:text-2xl text-white">
                  <TbPlaneArrival className="text-red-800" />
                  {cidadeDestino} / {estadoDestino}{" "}
                </p>
              </div>
              <div className="flex flex-col gap-2 ml-auto sm:ml-0">
                <p className="flex gap-2 text-xl sm:text-5xl text-green-400">
                  {formataValor(
                    ((retornaValorSoldo(pg, anoFilter)! *
                      (disp + locEsp + mil + hab)) /
                      100 +
                      retornaValorSoldo(pg, anoFilter)! +
                      (retornaValorSoldo(pgCo, anoFilter)! * compOrg) / 100) *
                ...
                <p className="flex gap-2 text-sm sm:text-2xl text-white ml-auto">
                  {dist} KM
                </p>
              </div>
            </div>
          ...
          <div className="border border-green-600 rounded-md p-6 relative mt-4">
            <h1 className="-top-4 absolute text-green-600 bg-gray-900 font-bold uppercase px-2 text-sm sm:text-lg">
              Valores
            </h1>
            <div className="border border-green-600 rounded-md p-6 relative mt-4">
              <h1 className="-top-4 absolute text-green-600 bg-gray-900 font-bold uppercase px-2 text-sm sm:text-lg">
              ...
              <div className="flex flex-1 flex-col sm:flex-row">
                ...
                <p className="pl-4 text-gray-600">
                  {formataValor(
                    (retornaValorSoldo(pg, anoFilter)! * hab) / 100
                  )}
                </p>
              </div>
              <div className="flex flex-1 flex-col sm:flex-row">
                ...
                <p className="pl-4 text-gray-600">
                  {formataValor(
                    (retornaValorSoldo(pg, anoFilter)! * mil) / 100
                  )}
                </p>
              </div>
              <div className="flex flex-1 flex-col sm:flex-row">
                ...
                <p className="pl-4 text-gray-600">
                  {formataValor(
                    (retornaValorSoldo(pg, anoFilter)! * locEsp) / 100
                  )}
                </p>
              </div>
              <div className="flex flex-1 flex-col sm:flex-row">
                ...
                <p className="pl-4 text-gray-600">
                  {formataValor(
                    (retornaValorSoldo(pg, anoFilter)! * disp) / 100
                  )}
                </p>
              </div>
              <div className="flex flex-1 flex-col sm:flex-row">
