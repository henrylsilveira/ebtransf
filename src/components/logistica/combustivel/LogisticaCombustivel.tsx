...
export function LogisticaCombustivel({ logistica, idComb, tipo }: { logistica: LogisticaCombustivelProps[], idComb: string, tipo: string }) {
    ...
    const handleSubmitCombustivel = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        if (formData.tipo == "" || formData.finalidade == "" || formData.quantidade == 0 || formData.data == "") {
            ...
            try {
                ...
                logistica.push(JSON.parse(registros));
                        ...
                        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                            ...
                            <tbody className="overflow-y-auto">
                                {logistica?.filter(log => log.idCombustivel === idComb).map((log, index) => (
                                    <tr key={index} className={`odd:bg-white odd:dark:bg-gray-900 dark:hover:bg-green-700/30 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-b-gray-700 ${log.tipo === "entrada" ? 'dark:border-l-4 dark:border-l-green-500' : 'dark:border-l-4 dark:border-l-red-500'}`}>
