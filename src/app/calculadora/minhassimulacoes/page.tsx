...
export default function MinhasSimulacoes() {
    ...
    useEffect(() => {
        const registros = localStorage.getItem("transferencia")
        if (registros !== null) {
    ...
    return (
        <>
            <title>EBCalc - Minhas Simulações
            </title>
            ...
            <div className="flex flex-col mx-auto max-w-4xl w-10/12 sm:text-md text-sm shadow-container p-2 sm:p-10 rounded-lg mb-20 mt-4 h-screen">
                <div className="w-full flex justify-center flex-col mb-4">
                    <h1 className="text-green-600 font-bold uppercase text-2xl mx-auto mb-2">Minha tabela de transferências</h1>
                </div>
                {
                    transferencia.length === 0
                        ? <NotData textoComponent={"Não foi possível carregar as transferências salvas ou não você não tem nenhuma salva localmente."} />
                        :
                        ...
                        <div className="overflow-x-auto shadow-md sm:rounded-lg">
                            <div className="overflow-x-auto shadow-md sm:rounded-lg">
                                <table className="w-full text-xs sm:text-sm text-left text-gray-400">
                                    ...
                                    <tbody>
                                        {transferencia.map((transf: DadosTransferencia) =>
