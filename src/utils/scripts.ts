import {
  DadosTransferencia,
  EstadosCidadesCoordProps,
  FeedbackCidadesProps,
  ModeloProcessoProps,
} from "@/types/types";
import { dbSoldo, dependenteIR, impostoRenda, soldo } from "./valores";
...
export function retornaValorSoldo(pg: string, year: string, reajuste?: number) {
  const response =
    dbSoldo[year as keyof typeof dbSoldo]?.find((soldo) => soldo.codigo === pg)
      ?.soldo! * (reajuste ? reajuste / 100 + 1 : 1);
...
export function calcularDiferencaAtual(data: any, dataT?: any) {
  ...
  return {
    ...
    mes: difMes,
    dia: difDia,
    totalDias: difAno * 365 + difMes * 30 + difDia,
  };
}
...
export function removerObjetoPorID(
  array: ObjectProps[],
  id: string
): ObjectProps[] {
...
export function converterParaFormatoPadrao(dataISO: string) {
  ...
  const minutosFormatados: string =
...
export function getTimeRemainingForFiveMinutes(isoDateTime: string): number {
  ...
  const timeRemaining =
    fiveMinutesInMilliseconds - timeDifferenceInMilliseconds;
...
export function returnCountCities(cities: DadosTransferencia[]) {
  const arrayCities = cities.map(
    (city) => city.cidadeDestino + " | " + city.estadoDestino
  );
  const arrayCitiesO = cities.map(
    (city) => city.cidadeOrigem + " | " + city.estadoOrigem
  );
  ...
  return allCities.map((city) => {
    return {
      ...
      count:
        arrayCities.filter((c) => c === city).length +
        arrayCitiesO.filter((c) => c === city).length,
    };
  });
}
export function returnFeedbackCities(cities: FeedbackCidadesProps[]) {
  ...
  return allCities.map((city) => {
    const totalArrayCidades = cities.filter(
      (c) => c.cidade + " | " + c.estado === city
    ).length;
    return {
      city: city,
      sigla: estados.filter((estado) => estado.nome === city.split(" | ")[1])[0]
        .sigla,
      educacao:
        cities.reduce((accumulator, c) => {
          return c.cidade + " | " + c.estado === city
            ? accumulator + c.educacao
            : accumulator;
        }, 0) / totalArrayCidades,
      saude:
        cities.reduce((accumulator, c) => {
      ...
      trabalho:
        cities.reduce((accumulator, c) => {
          return c.cidade + " | " + c.estado === city
            ? accumulator + c.trabalho
            : accumulator;
        }, 0) / totalArrayCidades,
      seguranca:
        cities.reduce((accumulator, c) => {
          return c.cidade + " | " + c.estado === city
            ? accumulator + c.seguranca
            : accumulator;
        }, 0) / totalArrayCidades,
      infraEstrutura:
        cities.reduce((accumulator, c) => {
          return c.cidade + " | " + c.estado === city
            ? accumulator + c.infraEstrutura
            : accumulator;
        }, 0) / totalArrayCidades,
      pnr:
        cities.reduce((accumulator, c) => {
      ...
      custoVida:
        cities.reduce((accumulator, c) => {
          return c.cidade + " | " + c.estado === city
            ? accumulator + c.custoVida
            : accumulator;
        }, 0) / totalArrayCidades,
      batalhao:
        cities.reduce((accumulator, c) => {
          return c.cidade + " | " + c.estado === city
            ? accumulator + c.batalhao
            : accumulator;
        }, 0) / totalArrayCidades,
    };
  });
...
export function calculaImpostoRenda(
  valorBruto: number,
  descontos: number,
  dependentes: number
): {
  ...
  let faixa: { aliquota: number; deducao: number } = {
    aliquota: 0,
    deducao: 0,
  };
  ...
  for (let index = 0; index < impostoRenda.length; index++) {
    if (
      baseCalculo >= impostoRenda[index].de &&
      baseCalculo <= impostoRenda[index].ate
    ) {
      faixa = {
        aliquota: impostoRenda[index].aliquota,
        deducao: impostoRenda[index].deducao,
      };
    }
  }
...
export function dividirArray(
  arr: AllDocumentTypes[],
  tamanho: number
): AllDocumentTypes[][] {
  let resultado = [];
...
export function returnProgressBarValue(processo: ModeloProcessoProps) {
  const etapasConcluidas = processo.etapas.filter(
    (etapa) => etapa.situacao === true
  ).length;
